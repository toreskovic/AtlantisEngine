        ...
        if line.strip().startswith("DEF_CLASS("):
            ...
            while class_found == "" and temp_line > 0:
                ...
                if current_line.startswith("class "):
                    class_found = current_line.removeprefix("class ").split(" ")[0]
                if current_line.startswith("struct "):
                    class_found = current_line.removeprefix("struct ").split(" ")[0]
                temp_line -= 1
            #print("Line{}: {}".format(count, line.strip()))
            current_macros.append({"line": count, "type": "class", "class_name": class_found})
...
def find_typerefs(node):
    ...
    if current_filename.startswith("./src/") or node_file.startswith("./src/"):
        if node_file.startswith("./src/") and node_file.endswith(".h") and not node_file.endswith(".gen.h"):
            if current_filename != node_file:
                ...
                current_gen_filename = os.path.dirname(current_filename) + "/generated/" + os.path.basename(current_filename).removesuffix(".h") + ".gen.h"
...
tu = index.parse(filename, ["-Wall", "-I./build/_deps/raylib-src/src", "-I./src/"])
