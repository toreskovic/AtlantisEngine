...
struct AObject
{
    ...
    // or ignore it (and potentially reuse it when creating new entities /
    // components)
    bool _isAlive = true;
...
struct AComponent : public AObject
{
    ...
    virtual void OnAddedToEntity(AEntity* entity) { Owner = entity; }
    ...
    virtual void Deserialize(const nlohmann::json& json) override
    {
        for (auto& comp : json["Components"])
        {
            AComponent* component =
                GetComponentOfType(comp["Name"].get<std::string>());
...
struct AResourceHolder
{
    ...
    AResourceHolder(AWorld* world) { World = world; }
